...
class RevAddOnWeatherUpdate {
	private $plugin_url		 = 'https://codecanyon.net/item/slider-revolution-responsive-wordpress-plugin/2751380';
	private $remote_url_info = 'addons/revslider-weather-addon/revslider-weather-addon.php';
	private $plugin_slug	 = 'revslider-weather-addon';
	private $plugin_path	 = 'revslider-weather-addon/revslider-weather-addon.php';
	private $version;
	private $plugins;
	private $option;
	...
	protected function _check_updates($force_check = false){
		...
		$last_check = get_option('revslider_weather_addon-update-check');
		
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider_weather_addon-update-check', $last_check);
		}
		...
		if(time() - $last_check > 60 * 60 * 24 * 30 || $force_check == true){
			...
			if(isset($data->basic)){
				update_option('revslider_weather_addon-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
				
				update_option('revslider_weather_addon-latest-version', $data->full->version);
			}
		}
	...
	public function _retrieve_update_info(){
		...
		$rattr = array(
			...
			'version' => urlencode(REV_ADDON_WEATHER_VERSION)
		);

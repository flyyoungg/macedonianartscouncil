...
class RevAddOnBackupUpdate {
	private $plugin_url		 = 'https://codecanyon.net/item/slider-revolution-responsive-wordpress-plugin/2751380';
	private $remote_url_info = 'addons/revslider-backup-addon/revslider-backup-addon.php';
	...
	protected function _check_updates($force_check = false){
		...
		$last_check = get_option('revslider_backup_addon-update-check');

		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider_backup_addon-update-check', $last_check);
		}
		...
		if(time() - $last_check > 60 * 60 * 24 * 30 || $force_check == true){
			...
			if(isset($data->basic)){
				update_option('revslider_backup_addon-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
				
				update_option('revslider_backup_addon-latest-version', $data->full->version);
			}
		}

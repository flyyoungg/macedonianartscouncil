...
class RsAddOnFilmstripNotice {
	...
	public function enqueue_notice_script() {
		...
		wp_enqueue_script($this->txtDomain . '-notice', RS_FILMSTRIP_PLUGIN_URL . 'admin/assets/js/dismiss-admin-notice.js', array('jquery'), $this->version, true);
	...
	public function add_notice() {
		...
		switch($this->notice) {
			...
			case 'add_notice_version':
				$id = md5($this->noticeSlug . '_add_notice_activation');
				$this->notice = 'The ' . $this->title . ' Add-On requires Slider Revolution ' . RsAddOnFilmstripBase::MINIMUM_VERSION . 

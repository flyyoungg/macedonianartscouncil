...
if ( !function_exists( 'trx_addons_cpt_sport_load_responsive_styles' ) ) {
	add_action("wp_enqueue_scripts", 'trx_addons_cpt_sport_load_responsive_styles', 2000);
	function trx_addons_cpt_sport_load_responsive_styles() {
		if (trx_addons_is_on(trx_addons_get_option('debug_mode'))) {
			wp_enqueue_style( 'trx_addons-cpt_sport-responsive', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_CPT . 'sport/sport.responsive.css'), array(), null );
		}
	}
}
...
if ( !function_exists( 'trx_addons_cpt_sport_merge_styles_responsive' ) ) {
	...
	function trx_addons_cpt_sport_merge_styles_responsive($list) {
		$list[] = TRX_ADDONS_PLUGIN_CPT . 'sport/sport.responsive.css';
		return $list;
	}
}

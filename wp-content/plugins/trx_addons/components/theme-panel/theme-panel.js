...
jQuery(document).ready(function(){
	...
	jQuery('.trx_addons_theme_panel_section_form_field_submit input[type="submit"]').on('click', function(e) {
		var form = jQuery( this ).parents( 'form' ),
			regexp = new RegExp(TRX_ADDONS_STORAGE['email_mask']);
		if ( jQuery('[name="trx_addons_activate_theme_code"]').val() === '' ) {
	...
	// Welcome banners
	if ( jQuery('.trx_addons_theme_panel_banners_item.trx_banners_item').length > 0 ) {
		trx_addons_banners_rotator( '.trx_addons_theme_panel_banners' );
	}

	// Focused image block - trigger click on 'Enter' or 'Space'
	jQuery( '.trx_addons_theme_panel_themes_list:not(.inited)' ).addClass( 'inited' )
	...
	// Category selector
	jQuery('.trx_addons_theme_panel_section_recent_themes_category_selector select')
		.on( 'change', function() {
			jQuery.post(
				TRX_ADDONS_STORAGE['ajax_url'],
				{
					'action': 'trx_addons_get_recent_themes',
					'nonce': TRX_ADDONS_STORAGE['ajax_nonce'],
				...
				function(response) {
					...
					if ( rez.error !== '' ) {
						...
						jQuery( '.trx_addons_theme_panel_themes_list' )
							.empty()
							.append( rez.data );
						if ( jQuery( '.trx_addons_theme_panel_themes_list > .trx_addons_image_block' ).length == 9 ) {
							jQuery('.trx_addons_theme_panel_themes_list_load_more').data( 'next-page', 2 ).show();
						} else {
							jQuery('.trx_addons_theme_panel_themes_list_load_more').data( 'next-page', 0 ).hide();
						}
					}
	...
	// Load more ecent themes
	jQuery('.trx_addons_theme_panel_themes_list_load_more')
		.on( 'click', function(e) {
			var load_more = jQuery(this);
			load_more.find('span[class*="trx_addons_icon"]').addClass('trx_addons_loading');
			jQuery.post(
				TRX_ADDONS_STORAGE['ajax_url'],
				{
					'action': 'trx_addons_get_recent_themes',
					'nonce': TRX_ADDONS_STORAGE['ajax_nonce'],
					'category': jQuery( '.trx_addons_theme_panel_section_recent_themes_category_selector select' ).val(),
					'page': parseInt( load_more.data('next-page'), 10 )
				},
				function(response) {
					var rez  = { error: '', data: '', nextPage: '' };
					load_more.find('span[class*="trx_addons_icon"]').removeClass('trx_addons_loading');
					if (response !== '' &&  response !== 0) {
						try {
							rez = JSON.parse( response );
						} catch (e) {
					...
					if ( rez.error !== '' ) {
						...
						jQuery( '.trx_addons_theme_panel_themes_list' )
							.append( rez.data );
						load_more.data('next-page', rez.next_page);
						if ( rez.next_page === 0 ) {
							load_more.fadeOut();
						}
					}

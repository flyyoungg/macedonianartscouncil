		...
		?><div class="trx_importer_sys_info">
			...
			if ( ! $checked ) {
				...
				<div class="trx_importer_sys_info_check_result trx_addons_info_box trx_addons_info_box_warning">
					<p><?php
						echo wp_kses_data(
								__("It seems that your server doesn't correspond with the theme requirements. You may encounter problems during the demo data installation.", 'trx_addons')
							);
					?></p>
					<p><?php
						echo wp_kses_post(
								sprintf(
									__("You may want to check with your Hosting Provider if they can fix the issues for you. Or consider using %s that provide 50%% discount for our customers.", 'trx_addons'),
					...
					<li><?php
						echo wp_kses_post(
							sprintf(
								...
								'<a href="' . esc_url( trailingslashit( trx_addons_get_protocol() . ':' . apply_filters( 'trx_addons_filter_get_theme_data', '', 'theme_demofiles_url' ) ) . 'demo.zip' ) . '" target="_blank">' . esc_html__( 'this archive', 'trx_addons' ) . '</a>'
							) );
						?></li>
					<li><?php
						echo wp_kses_post(

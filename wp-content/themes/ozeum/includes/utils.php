...
if ( ! function_exists( 'ozeum_array_get_first' ) ) {
	function ozeum_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'ozeum_array_get_keys_by_value' ) ) {
	function ozeum_array_get_keys_by_value( $arr, $value = 1, $as_string = true ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'ozeum_array_delete_by_value' ) ) {
	function ozeum_array_delete_by_value( $arr, $value ) {
		do {
...
if ( ! function_exists( 'ozeum_array_from_list' ) ) {
	function ozeum_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'ozeum_array_merge' ) ) {
	function ozeum_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'ozeum_array_insert' ) ) {
	function ozeum_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = ozeum_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = ozeum_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );
...
// @param mixed $stuff one or more arrays or scalars to be inserted into $haystack
// @return int the index at which $needle was found
if ( ! function_exists( 'ozeum_array_before' ) ) {
	function ozeum_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'ozeum_get_tag' ) ) {
	function ozeum_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'ozeum_get_tag_attrib' ) ) {
	function ozeum_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'ozeum_get_css_position_from_values' ) ) {
	function ozeum_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( ozeum_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'ozeum_prepare_css_value' ) ) {
	function ozeum_prepare_css_value( $val ) {
		if ( '' != $val ) {
			$ed = substr( $val, -1 );
			if ( '0' <= $ed && $ed <= '9' ) {
				$val .= 'px';
...
if ( ! function_exists( 'ozeum_parse_icons_classes' ) ) {
	function ozeum_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'ozeum_stripslashes' ) ) {
	function ozeum_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
// Get GET, POST value
if ( ! function_exists( 'ozeum_get_value_gp' ) ) {
	function ozeum_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'ozeum_get_value_gpc' ) ) {
	function ozeum_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'ozeum_get_value_gps' ) ) {
	function ozeum_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'ozeum_get_session_value' ) ) {
	function ozeum_get_session_value( $name, $defa = '' ) {
		global $wp_session;
...
if ( ! function_exists( 'ozeum_set_session_value' ) ) {
	function ozeum_set_session_value( $name, $value ) {
		global $wp_session;
...
if ( ! function_exists( 'ozeum_del_session_value' ) ) {
	function ozeum_del_session_value( $name ) {
		global $wp_session;
...
if ( ! function_exists( 'ozeum_hex2rgb' ) ) {
	function ozeum_hex2rgb( $hex ) {
...
if ( ! function_exists( 'ozeum_hex2rgba' ) ) {
	function ozeum_hex2rgba( $hex, $alpha ) {
		$rgb = ozeum_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'ozeum_hex2hsb' ) ) {
	function ozeum_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = ozeum_rgb2hsb( ozeum_hex2rgb( $hex ) );
...
if ( ! function_exists( 'ozeum_rgb2hsb' ) ) {
	function ozeum_rgb2hsb( $rgb ) {
		$hsb      = array();
...
if ( ! function_exists( 'ozeum_hsb2rgb' ) ) {
	function ozeum_hsb2rgb( $hsb ) {
		$rgb = array();
		$h   = round( $hsb['h'] );
...
if ( ! function_exists( 'ozeum_rgb2hex' ) ) {
	function ozeum_rgb2hex( $rgb ) {
		$hex = array(
			dechex( $rgb['r'] ),
			dechex( $rgb['g'] ),
			dechex( $rgb['b'] ),
...
if ( ! function_exists( 'ozeum_hsb2hex' ) ) {
	function ozeum_hsb2hex( $hsb ) {
		return ozeum_rgb2hex( ozeum_hsb2rgb( $hsb ) );
	}
}
...
if (!function_exists('ozeum_date_to_sql')) {
	function ozeum_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
// Display price
if (!function_exists('ozeum_format_price')) {
	function ozeum_format_price($price) {
...
// Convert number to K
if (!function_exists('ozeum_num2kilo')) {
	function ozeum_num2kilo($num) {
...
if (!function_exists('ozeum_size2num')) {
	function ozeum_size2num($size) {
...
if (!function_exists('ozeum_parse_num')) {
	function ozeum_parse_num($str) {
...
if ( ! function_exists( 'ozeum_prepare_macros' ) ) {
	function ozeum_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'ozeum_remove_macros' ) ) {
	function ozeum_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'ozeum_is_on' ) ) {
	function ozeum_is_on( $prm ) {
		return $prm > 0 || in_array( strtolower( $prm ), array( 'true', 'on', 'yes', 'show' ) );
	}
}
if ( ! function_exists( 'ozeum_is_off' ) ) {
	function ozeum_is_off( $prm ) {
		return empty( $prm ) || 0 === $prm || in_array( strtolower( $prm ), array( 'false', 'off', 'no', 'none', 'hide' ) );
	}
}
if ( ! function_exists( 'ozeum_is_inherit' ) ) {
	function ozeum_is_inherit( $prm ) {
		return in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

// Return truncated string (by chars number)
if ( ! function_exists( 'ozeum_strshort' ) ) {
	function ozeum_strshort( $str, $maxlength, $add = '&hellip;' ) {

		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = ozeum_strip_tags( $str );
...
if ( ! function_exists( 'ozeum_strip_tags' ) ) {
	function ozeum_strip_tags( $str ) {
...
if ( ! function_exists( 'ozeum_excerpt' ) ) {
	function ozeum_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return ozeum_strwords( ozeum_strip_tags( $str ), $maxlength, $add );
	}
}


// Return truncated string (by words number)
if ( ! function_exists( 'ozeum_strwords' ) ) {
	function ozeum_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
// Unserialize string
if ( ! function_exists( 'ozeum_unserialize' ) ) {
	function ozeum_unserialize( $str ) {
...
if ( ! function_exists( 'ozeum_str_replace' ) ) {
	function ozeum_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = ozeum_str_replace( $from, $to, $v );
			}
			...
			$str = ozeum_unserialize( $str );
			$str = is_array( $str ) || is_object( $str )
							? serialize( ozeum_str_replace( $from, $to, $str ) )
...
if ( ! function_exists( 'ozeum_str_replace_once' ) ) {
	function ozeum_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'ozeum_tags_count' ) ) {
	function ozeum_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'ozeum_getimagesize' ) ) {
	function ozeum_getimagesize( $url ) {
		// Remove scheme from url
		$url = ozeum_remove_protocol_from_url( $url );
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => ozeum_remove_protocol_from_url( $upload_info['baseurl'] ),
			),
			'child'   => array(
				'dir' => OZEUM_CHILD_DIR,
				'url' => ozeum_remove_protocol_from_url( OZEUM_CHILD_URL ),
			),
			'theme'   => array(
				'dir' => OZEUM_THEME_DIR,
				'url' => ozeum_remove_protocol_from_url( OZEUM_THEME_URL ),
			),
		);
...
if ( ! function_exists( 'ozeum_clear_thumb_size' ) ) {
	function ozeum_clear_thumb_size( $url ) {
		$pi            = pathinfo( $url );
		$pi['dirname'] = ozeum_remove_protocol_from_url( $pi['dirname'], false );
...
if ( ! function_exists( 'ozeum_add_thumb_size' ) ) {
	function ozeum_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
			...
			if ( empty( $_wp_additional_image_sizes[ $thumb_size ]['height'] ) || empty( $_wp_additional_image_sizes[ $thumb_size ]['crop'] ) ) {
				...
				if ( $image_id > 0 ) {
					...
					if ( ! empty( $attach[0] ) ) {
						...
						$pi['dirname'] = ozeum_remove_protocol_from_url( $pi['dirname'] );
						$parts = explode( '-', $pi['filename'] );
					}
				}
			} else {
		...
		$new_url = ozeum_remove_protocol_from_url( $pi['dirname'] . '/' . $pi['filename'] . '.' . $pi['extension'] );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = ozeum_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = ozeum_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = ozeum_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'ozeum_get_thumb_size' ) ) {
	function ozeum_get_thumb_size( $ts ) {
		$retina = ozeum_get_retina_multiplier() > 1 ? '-@retina' : '';
		return ( 'post-thumbnail' == $ts ? '' : 'ozeum-thumb-' ) . $ts . $retina;
	}
}
...
if ( ! function_exists( 'ozeum_get_closest_thumb_size' ) ) {
	function ozeum_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'ozeum_get_attachment_url' ) ) {
	function ozeum_get_attachment_url( $image_id, $size = 'full' ) {
		if ( $image_id > 0 ) {
			$attach   = wp_get_attachment_image_src( $image_id, $size );
			$image_id = isset( $attach[0] ) && '' != $attach[0] ? $attach[0] : '';
		} else {
			$image_id = ozeum_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}
...
if ( ! function_exists( 'ozeum_get_post_image' ) ) {
	function ozeum_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'ozeum_get_post_audio' ) ) {
	function ozeum_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		...
		if ( $src ) {
			...
			$img = ozeum_get_tag( $post_text, '<audio', '</audio>' );
			if ( empty( $img ) ) {
				$img = do_shortcode( ozeum_get_tag( $post_text, '[audio', '[/audio]' ) );
			}
			if ( empty( $img ) ) {
				$img = ozeum_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'ozeum_get_post_video' ) ) {
	function ozeum_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		...
		if ( $src ) {
			...
			$img = ozeum_get_tag( $post_text, '<video', '</video>' );
			if ( empty( $img ) ) {
				$sc = ozeum_get_tag( $post_text, '[video', '[/video]' );
				if ( empty( $sc ) ) {
					$sc = ozeum_get_tag( $post_text, '[trx_widget_video', '' );
				}
				if ( ! empty( $sc ) ) {
					$img = do_shortcode( $sc );
				}
				if ( empty( $img ) && preg_match_all( '/<!\\-\\- wp\\:trx-addons\\/video.+"link"\\:"([^"]*)"/i', $post_text, $matches ) ) {
					$img = ozeum_get_embed_video( $matches[1][0] );
				}
			}
		}
		return $img;
...
if ( ! function_exists( 'ozeum_get_post_iframe' ) ) {
	function ozeum_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = ozeum_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = ozeum_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'ozeum_filter_get_post_iframe', $img );
	}
}


// Add 'autoplay' feature in the video
if ( ! function_exists( 'ozeum_make_video_autoplay' ) ) {
	function ozeum_make_video_autoplay( $video ) {
...
if ( ! function_exists( 'ozeum_get_embed_video' ) ) {
	function ozeum_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			$embed_video = ozeum_make_video_autoplay( $embed_video );
		} else {
			$video = str_replace( '/watch?v=', '/embed/', $video );
			$video = ozeum_add_to_url(
				$video,
				array(
					'feature'        => 'oembed',
					'controls'       => 0,
...
// Return layout with embeded video
if ( ! function_exists( 'ozeum_get_embed_video_tag_name' ) ) {
	function ozeum_get_embed_video_tag_name() {
...
if ( ! function_exists( 'ozeum_is_from_uploads' ) ) {
	function ozeum_is_from_uploads( $url ) {
		$url          = ozeum_remove_protocol_from_url( $url );
		$uploads_info = wp_upload_dir();
		$uploads_url  = ozeum_remove_protocol_from_url( $uploads_info['baseurl'] );
		$uploads_dir  = $uploads_info['basedir'];
...
if ( ! function_exists( 'ozeum_is_youtube_url' ) ) {
	function ozeum_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

// Check if URL from Vimeo
if ( ! function_exists( 'ozeum_is_vimeo_url' ) ) {
	function ozeum_is_vimeo_url( $url ) {
...
------------------------------------------------------------------- */
if ( ! function_exists( 'ozeum_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'ozeum_init_filesystem', 0 );
	function ozeum_init_filesystem() {
...
if ( ! function_exists( 'ozeum_fpc' ) ) {
	function ozeum_fpc( $file, $data, $flag = 0 ) {
		...
		if ( ! empty( $file ) ) {
			if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
				...
				if ( ozeum_is_on( ozeum_get_theme_option( 'debug_mode' ) ) ) {
					...
					throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'ozeum' ), $file ) );
				}
			}
		}
		return false;
...
if ( ! function_exists( 'ozeum_fgc' ) ) {
	function ozeum_fgc( $file ) {
		global $wp_filesystem;
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				if ( ozeum_is_on( ozeum_get_theme_option( 'debug_mode' ) ) ) {
					// Translators: Add the file name to the message
					throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'ozeum' ), $file ) );
				}
			}
		}
		return '';
...
if ( ! function_exists( 'ozeum_fga' ) ) {
	function ozeum_fga( $file ) {
		global $wp_filesystem;
		if ( ! empty( $file ) ) {
			if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
				$file = str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
				...
				if ( ozeum_is_on( ozeum_get_theme_option( 'debug_mode' ) ) ) {
					// Translators: Add the file name to the message
					throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'ozeum' ), $file ) );
				}
			}
		}
		return array();
...
if ( ! function_exists( 'ozeum_retrieve_json' ) ) {
	function ozeum_retrieve_json( $url ) {
		return function_exists( 'trx_addons_retrieve_json' ) ? trx_addons_retrieve_json( $url ) : '';
	}
}

// Remove unsafe characters from file/folder path
if ( ! function_exists( 'ozeum_esc' ) ) {
	function ozeum_esc( $name ) {
...
if ( ! function_exists( 'ozeum_check_min_file' ) ) {
	function ozeum_check_min_file( $file, $dir ) {
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && ozeum_is_off( ozeum_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && ozeum_is_off( ozeum_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'ozeum_get_file_dir' ) ) {
	function ozeum_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! OZEUM_ALLOW_SKINS && ! ozeum_get_theme_setting( 'check_min_version' ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'ozeum_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( OZEUM_CHILD_DIR != OZEUM_THEME_DIR && file_exists( OZEUM_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? OZEUM_CHILD_URL : OZEUM_CHILD_DIR ) . ozeum_check_min_file( $file, OZEUM_CHILD_DIR );
			} elseif ( file_exists( OZEUM_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? OZEUM_THEME_URL : OZEUM_THEME_DIR ) . ozeum_check_min_file( $file, OZEUM_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'ozeum_get_file_url' ) ) {
	function ozeum_get_file_url( $file ) {
		return ozeum_get_file_dir( $file, true );
	}
}

// Return file extension from full name/path
if ( ! function_exists( 'ozeum_get_file_ext' ) ) {
	function ozeum_get_file_ext( $file ) {
...
if ( ! function_exists( 'ozeum_get_file_name' ) ) {
	function ozeum_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
...
if ( ! function_exists( 'ozeum_get_folder_dir' ) ) {
	function ozeum_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'ozeum_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( OZEUM_CHILD_DIR != OZEUM_THEME_DIR && is_dir( OZEUM_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? OZEUM_CHILD_URL : OZEUM_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( OZEUM_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? OZEUM_THEME_URL : OZEUM_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'ozeum_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'ozeum_get_folder_url' ) ) {
	function ozeum_get_folder_url( $folder ) {
		return ozeum_get_folder_dir( $folder, true );
	}
}


// Merge all separate styles and scripts to the single file to increase page upload speed
if ( ! function_exists( 'ozeum_merge_js' ) ) {
	function ozeum_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= ozeum_fgc( ozeum_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			ozeum_fpc(
				ozeum_get_file_dir( $to ),
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'ozeum' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'ozeum_filter_js_output', apply_filters( 'ozeum_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}
...
if ( ! function_exists( 'ozeum_merge_css' ) ) {
	function ozeum_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'ozeum_filter_responsive_sizes', ozeum_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = ozeum_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = ozeum_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'ozeum_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'ozeum_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'ozeum' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'ozeum' ) );
		...
		if ( '' != $output ) {
			ozeum_fpc(
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'ozeum') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'ozeum_filter_css_output', apply_filters( 'ozeum_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}
...
// Merge styles to the CSS file
if ( ! function_exists( 'ozeum_filter_merge_css' ) ) {
	add_filter( 'ozeum_filter_merge_css', 'ozeum_filter_merge_css' );
	function ozeum_filter_merge_css( $css ) {
		return str_replace( '../../', '../', $css);
	}
}

// Merge styles to the SASS file
if ( ! function_exists( 'ozeum_merge_sass' ) ) {
	function ozeum_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'ozeum_filter_responsive_sizes', ozeum_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'ozeum_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			ozeum_fpc( ozeum_get_file_dir( $to ), apply_filters( 'ozeum_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'ozeum_remove_protocol_from_url' ) ) {
    function ozeum_remove_protocol_from_url( $url, $complete = true ) {
        return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
    }
}

// Add parameters to URL
if ( ! function_exists( 'ozeum_add_to_url' ) ) {
	function ozeum_add_to_url( $url, $prm ) {

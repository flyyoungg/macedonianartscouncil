...
class baseOptionsGenerator {
	...
	function superlink($item) {
		...
		if(in_array('portfolio',$shows)){
			...
			$numberposts=theme_get_option('advanced','superlink_max_portfolios');
			$orderby= theme_get_option('advanced','superlink_orderby_portfolios');
			$order= theme_get_option('advanced','superlink_order_portfolios');
			echo '<option value="">Select Portfolio</option>';
			foreach($this->get_select_target_options('portfolio',$numberposts,$orderby,$order) as $key => $option) {
				echo '<option value="' . $key . '"';
		...
		if(in_array('post',$shows)){
			...
			$numberposts=theme_get_option('advanced','superlink_max_posts');
			$orderby= theme_get_option('advanced','superlink_orderby_posts');
			$order= theme_get_option('advanced','superlink_order_posts');
			echo '<select name="' . $id . '_post" id="' . $id . '_post" ' . $hidden . '>';
			echo '<option value="">Select Post</option>';
			foreach($this->get_select_target_options('post',$numberposts,$orderby,$order) as $key => $option) {
				echo '<option value="' . $key . '"';
	...
	function get_select_target_options($type,$numberofposts=-1, $orderby='title',$order='ASC') {
		$options = array();
		switch($type){
			case 'revslider':
			...
			case 'author':
				...
				break;
			case 'post':
				$entries = get_posts('orderby='.$orderby.'&numberposts='.$numberofposts.'&order='.$order.'&suppress_filters=0');
				foreach($entries as $key => $entry) {
					$options[$entry->ID] = $entry->post_title;
				}
				break;
			case 'portfolio':
				$entries = get_posts('post_type=portfolio&orderby='.$orderby.'&numberposts='.$numberofposts.'&order='.$order.'&suppress_filters=0');
				foreach($entries as $key => $entry) {
					$options[$entry->ID] = $entry->post_title;
				}
				break;

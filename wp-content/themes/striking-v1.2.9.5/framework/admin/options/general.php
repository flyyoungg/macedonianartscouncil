			...
			array(
				...
				"options" => array(
					...
					array(
						...
						"desc" => sprintf(__('<p> The &#34;ON&#34; position of this setting activates display of a <a href="%s/wp-admin/options-general.php" target="_blank">Tagline</a> - an additional brief line of descriptive text often used for catchphrase or slogan purposes, that appears just below the Site Title.</p>','theme_admin'),site_url()),		
...
<p><strong>1 -</strong> <u>to show the wpml flags</u> if using the wordpress multi-language plugin ->Striking has auto language detection coded into it for wpml so that the wpml flag widget is not necessary if one chooses this option.<br /><br /><strong>2 -</strong> <u>to set it as a Widget Area</u> in which case after saving this panel go to <a href='%s/wp-admin/widgets.php' target='_blank'>Appearence -> Widgets</a> and a new Header Widget Area Sidebar will appear below the other sidebar areas to which widgets can be inserted.<br /><br /><strong>3 -</strong> <u>set the area to the html mode</u> in which case one can style the <b>Header Top Area Html Code</b> field below to hold whatever content and styling desired. &nbsp;&nbsp;The html mode was included into Striking at the request of designers who use the theme for their client sites, and is only recommended for users with a good understanding of html coding. &nbsp;&nbsp;However, there are a number of threads in the Striking support forum on styling the header widget area in html mode which may assist the novice coder on styling this area.</p><p><em>HINT 1:</em>&nbsp;&nbsp; If one wishes to have different widgets or content side by side in the header widget area, it should be set to html mode, and with use of divs, or column or layout shortcodes, and then place your content, and widget shortcodes into the columns.</p><p><em>HINT 2:</em>&nbsp;&nbsp; An easy way to build the content for this field when in HTML mode is to do so in a page content editor, and then cut and paste it into this field.</p>",'theme_admin'),site_url()),
			...
			array(
				...
				"options" => array(
					...
					array(
						"name" => __("Sub Level Navigation Arrows ",'theme_admin'),
						"desc" => __("<p>If this setting is toggled <em>ON</em>, the navigation will display a arrow pointing down on the side of each sub menu text if there are sub-navigation children. &nbsp;&nbsp;So each arrow provide a visual cue to the site viewer that there are child pages for a sub level navigation item.</p>",'theme_admin'),
						"id" => "nav_arrow_sub",
						"default" => true,
						"type" => "toggle"
					),
			...
			array(
				"slug" => 'consent',
				"name" => __("GDPR Consent",'theme_admin'),
				"options" => array(
					array(
						"name" => __("Contact Form Consent Text",'theme_admin'),
						"desc" => __("<p>In here you can add the consent text for submitting a Contact Form. This is required within the EU since may 25, 2018. If left empty there will be no checkbox in the form. If any text below is entered a checkbox with that text will show in the contact form to which the user can confirm the use of his data.</p>",'theme_admin'),
						"id" => "gdpr_consent",
						"default" => "I give permission to store the above data and use it to contact me.",
						"type" => "textarea",
						'rows' => '2',
					),
				),
...
<code>&lt;script type=&quot;text/javascript&quot;&gt;alert("hello world");&lt;/script&gt;</code></p>','theme_admin'),site_url()),
						"id" => "custom_js",
						"default" => "",
						'rows' => '10',
						"type" => "textarea"

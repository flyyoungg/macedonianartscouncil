...
function theme_enqueue_scripts() {
	...
	if( ! empty( $gmap_api_key ) ){
		$gmap_url_with_api_key = add_query_arg( 'key', urlencode( $gmap_api_key ), "https://maps.googleapis.com/maps/api/js");
		wp_register_script('gmap-api-loader',$gmap_url_with_api_key,array('jquery'),'2.1.5', $move_bottom); 	
	}else{
		wp_register_script('gmap-api-loader','https://maps.googleapis.com/maps/api/js',array('jquery'),'2.1.5', $move_bottom); 
	}	
	...
	if(class_exists( 'Essential_Grid' )){
		if(get_option('tp_eg_use_lightbox') !== 'disabled') {
			$ess_grid_version=Essential_Grid::VERSION;
			if ( version_compare($ess_grid_version, "2.2.1", '>=')) {
				wp_dequeue_script( 'themepunchboxext' );
				wp_deregister_script( 'themepunchboxext' );
				wp_register_script( 'themepunchboxext', THEME_JS.'/essgrid/jquery.esgbox.min.js', array('jquery'), false, $move_bottom);
